...
helpFunction()
{
   echo "Usage: main_minimap2_run.sh -d /path/to/the/data -k kit-name -s /path/to/the/script -m 1400 -M 1800 -i 85 -c 85"
...
while getopts "d:k:s:m:M:i:c:" opt
do
    case "$opt" in
    d )
        data_path="$OPTARG"
...
if [ "$(cat $data_path/barcode_list | wc -l)" -eq 0 ]; then
    ...
    do
    	...
    	echo "bash $script_path/real_time_analysis_minimap2.sh -d $data_path -k $kit_name -b $barcode -m $min -M $max -i $identity -c $coverage"
