cohort_analysis_plots <- reactive({
    ...
    rel_abundance_val(rel_abundance_data)
    ...
    fun_heatmap_plot <- function(matrix)
    {
        ...
        col_fun <- colorRamp2(c(2, 0, -6), c("#FFD700", "#1010fe", "#FF1212"))
    ...
    return(list(
      # stacked_barplot = fun_stacked_bar_plot(rel_abundance_data, sample_metadata, lineage),
...
observeEvent(c(input$upload_data, input$taxa), ignoreNULL=TRUE, ignoreInit=TRUE, {
    ...
    output$plot_stacked_barplot <- renderPlotly({
        ...
        if(mode() == "Offline")
        {
          req(rel_abundance_val(), input$taxa, input$top_taxa)
        
          stacked_df <- rel_abundance_val()
        ...
        {
          req(real_rel_abundance_val(), input$taxon_select, input$top_taxa)

          stacked_df <- real_rel_abundance_val()
    ...
    output$plot_boxplot <- renderPlot({
      if(mode() == "Offline")
      {
        plots_data$alpha_diversity_plot
      }
      else if(mode() == "Realtime")
      {
        req(real_rel_abundance_filtered_val(), input$taxon_select, sample_metadata)

        alpha_diversity_data <- real_rel_abundance_filtered_val()
    ...
    output$plot_pcoa <- renderPlot({
      if(mode() == "Offline")
      {
        plots_data$pcoa_plot
      }
      else if(mode() == "Realtime")
      {
        req(real_abundance_matrix_val(), sample_metadata)

        matrix <- real_abundance_matrix_val()
    ...
    output$plot_heatmap <- renderPlot({
      if(mode()=="Offline")
      {
        plots_data$heatmap_plot
      }
      else if(mode()=="Realtime")
      {
        req(real_rel_abundance_filtered_matrix_val(), sample_metadata)

        matrix <- real_rel_abundance_filtered_matrix_val()
    ...
    output$download_permanova_csv <- downloadHandler(
      filename = paste0("PERMANOVA_Result_", Sys.Date(), ".tsv"),
      
      content = function(file) {
        if(mode()=="Offline")
        {
          write.tsv(plots_data$permanova_res, file, row.names = FALSE)
        }
        else if(mode()=="Realtime")
        {
          write.tsv(table_permanova(), file, row.names = FALSE)
        }
      }
    )
})